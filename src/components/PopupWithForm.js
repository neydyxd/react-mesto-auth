    ...
    return(
        ...
        <div className={popupClass}  >
            <div className="popup__container" onClick={(evt) => evt.stopPropagation()}>
                ...
                <form onSubmit={onSubmit} name={name} className={`popup__form popup__form_${name}`} noValidate>
                    <h2 className="popup__title">{title}</h2>
                    {children}
                    <button  className="popup__save popup__save_edit" type="submit">{button}</button>
                </form>
            </div>
        </div>
        </>
